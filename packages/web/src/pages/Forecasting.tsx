...
export default function Forecasting() {
  ...
  const [tab, setTab] = useState<'series' | 'scenarios'>('series');
  ...
  return (
    <>
      <PageHeader
        title="Forecasting"
        description="Project a series forward from recorded actuals, or vary the volume drivers underneath it and compare the cases."
      />

      <Tabs
        label="Forecasting views"
        active={tab}
        onChange={setTab}
        tabs={[
          ...
          {result && result.warnings.length > 0 ? (
            ...
            >
              <p className="text-sm font-semibold">
                {result.warnings.length} warning{result.warnings.length === 1 ? '' : 's'} from this
                run
              </p>
                      ...
                      {result.periodKeys.map((periodKey, index) => (
                        <tr key={periodKey}>
                          ...
                          <td className="num font-medium">
                            {money0(result.point[index], currency)}
                          </td>
                          <td className="num">
                        ...
                        {result.candidates.map((candidate) => {
                          ...
                          return (
                            <tr
                              ...
                              className={
                                selected ? 'bg-accent-50 dark:bg-accent-900/20' : undefined
                              }
                            >
                              <td>
              ...
              <Card title="Fit diagnostics" subtitle="In-sample quality over the fitted window">
                <dl className="grid grid-cols-2 gap-3 text-xs sm:grid-cols-3">
                  ...
                  <div>
                    ...
                    <dd className="tabular-nums font-medium">
                      {decimal(result.accuracy.rSquared)}
                    </dd>
                  </div>
                  <div>
